...
public class GeminiApiClient {
    ...
    public Mono<String> getJudgmentResult(Map<String, String> situations) {
        ...
        String prompt = """
            ...
            ### 예시 목록
            A: 룸메 B가 컵을 안 씻어서 매번 내가 씻어야 했음.
            B: 컵은 곧 씻으려던 참이었는데 나보고 뭐라 함.
            C: 난 내 컵은 내가 씻었음.
            D: (입장 없음)
            → (A: 0%, B: 90%, C: 0%, D: 10%)
            (해설: B의 잘못이 명백하며, D는 입장 표명이 없어 책임이 경미하지만 공동 생활에 무관심한 측면이 반영될 수 있습니다.)
            ...
            D: (입장 없음)
            → (A: 80%, B: 0%, C: 20%, D: 0%)
            (해설: A가 가장 큰 잘못을 했지만, B와 C가 서로 소통이 부족했던 점도 고려.)
            
            ### 에러 응답 규칙 안내
            아래 상황 중 하나에 해당하면 판단을 유보하고 지정된 메시지를 반환하십시오:
            
            1. A, B 중 한 명이라도 입장이 너무 짧거나 불명확한 경우:
